...
function init3D() {
    ...
    // Central Complex Structure (Precision Gear)
    const createGearShape = (innerRadius, outerRadius, teeth, toothDepth) => {
        const shape = new THREE.Shape();
        const toothWidthAngle = (Math.PI * 2) / (teeth * 4);
        for (let i = 0; i < teeth; i++) {
            const baseAngle = (i / teeth) * Math.PI * 2;
            const a1 = baseAngle - toothWidthAngle * 0.77;
            const a2 = baseAngle - toothWidthAngle * 0.3;
            const a3 = baseAngle + toothWidthAngle * 0.3;
            const a4 = baseAngle + toothWidthAngle * 0.77;
            const rIn = outerRadius - toothDepth;
        ...
        const spokeRadius = (innerRadius + (outerRadius - toothDepth)) / 2;
        for (let i = 0; i < 5; i++) {
            const holeAngle = (i / 5) * Math.PI * 2;
            const hole = new THREE.Path();
            hole.absarc(Math.cos(holeAngle) * spokeRadius * 0.9, Math.sin(holeAngle) * spokeRadius * 0.9, 0.35, 0, Math.PI * 2, true);
            shape.holes.push(hole);
        }
        return shape;
    };

    const gearShape = createGearShape(0.8, 2.6, 14, 0.5);
    const extrudeSettings = { depth: 0.7, bevelEnabled: true, bevelThickness: 0.1, bevelSize: 0.08, bevelSegments: 4 };
    ...
    // Add Clean CAD-style Blueprint Lines
    const edges = new THREE.EdgesGeometry(geometry, 25);
    const lineMat = new THREE.LineBasicMaterial({ color: 0x0EA5E9, transparent: true, opacity: 0.6 });
    model.add(new THREE.LineSegments(edges, lineMat));
...
function animate() {
    ...
    if (animationGroup) {
        animationGroup.rotation.y += 0.003;
        animationGroup.position.y = Math.sin(time * 10) * 0.1; // Floats slightly
...
const productData = {
    "Tool Designing": {
        cat: "ENGINEERING",
        desc: "Expert CAD/CAM tool designing utilizing advanced subtractive and additive modeling. We specialize in precision tool architecture for automotive and aerospace sectors.",
        specs: [
            { label: "Software", value: "AutoCAD / SolidWorks" },
            { label: "Design Time", value: "3-5 Business Days" },
            { label: "Form Factor", value: "Complex Geometries" }
        ]
    },
    "Press Tools & Dies": {
        cat: "MANUFACTURING",
        desc: "High-tonnage press tools and dies engineered for durability and high-volume production. Our dies maintain exceptional fidelity across extended run cycles.",
        specs: [
            { label: "Material", value: "Hardened Tool Steel" },
            { label: "Type", value: "Progressive / Compound" },
            { label: "Life Span", value: "1 Million+ Hits" }
        ]
    },
    "Jigs & Fixtures": {
        cat: "PRECISION",
        desc: "Custom jigs and specialized fixtures designed to optimize your assembly lines. We provide solutions that guarantee repetitive accuracy for high-tempo manufacturing.",
        specs: [
            { label: "Tolerance", value: "±0.01 MM" },
            { label: "Materials", value: "Alm / Steel / Engineered Plastics" },
            { label: "Application", value: "Assembly / Welding / Inspection" }
        ]
    },
    "Sheet Metal Components": {
        cat: "PRODUCTION",
        desc: "Full-scale sheet metal component production. From blanking and piercing to complex forming, we deliver turnkey industrial parts for various global industries.",
        specs: [
            { label: "Thickness", value: "0.2mm to 6mm" },
            { label: "Finish", value: "Zinc / Chrome / Powder Coating" },
            { label: "Volume", value: "Prototype to Mass Scale" }
        ]
    },
    "CNC VMC & Machined Components": {
        cat: "TECHNOLOGY",
        desc: "Cutting-edge 5-axis vertical machining center (VMC) capabilities. We deliver components that meet the strictest industrial tolerances and surface finish requirements.",
        specs: [
            { label: "Machining", value: "VMC / HMC / CNC Lathe" },
            { label: "Tolerance", value: "±0.005 MM" },
            { label: "Materials", value: "MS / SS / Alm / Brass / Titanium" }
        ]
    }
};
...
const closePanel = () => detailPanel.classList.remove('open');
